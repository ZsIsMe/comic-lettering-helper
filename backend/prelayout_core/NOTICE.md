...
`characters.py` adapts the character-overlay selection and ordering rules from `ctd_overlay_processor/processor.py` at the source commit above, without Qt imports. Web hover labels follow `ctd_overlay_processor/measure_view.py` (W/H pixels and estimated font size, with calculated-size fallback). Source outputs remain read-only; compact per-page caches are derived locally.
