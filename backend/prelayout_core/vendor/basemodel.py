...
from prelayout_core.vendor.utils.general import CUDA, DEVICE
from prelayout_core.vendor.network.yolov5.yolo import Model
import torch
import cv2
import numpy as np
from prelayout_core.vendor.network.yolov5.yolo import load_yolov5_ckpt
from prelayout_core.vendor.utils.yolov5_utils import fuse_conv_and_bn
import glob
import torch.nn as nn
from prelayout_core.vendor.utils.weight_init import init_weights
from prelayout_core.vendor.network.yolov5.common import C3, Conv
...
class TextDetector(nn.Module):
    ...
    def initialize_db(self, unet_weights):
        ...
        self.seg_net.load_state_dict(torch.load(unet_weights, map_location='cpu', weights_only=True)['weights'])
...
def get_base_det_models(model_path, device='cpu', half=False, act='leaky'):
    textdetector_dict = torch.load(model_path, map_location=device, weights_only=True)
