...
2026-09-24 本機迭代：工具列收緊並重新命名範圍按鈕，未設定頁面的預設範圍改為四邊內縮 10px。前後端均須更新以保持預覽與快照一致；既有 `repair_scope.json` 不修改、不遷移。

2026-09-24 手動四線作用範圍：前後端及任務輸入／回貼編排均有變更，需要同步後端與正式 frontend build；三套固定工作流 JSON 和模型參數未改。旧項目沒有 `repair_scope.json` 時預設關閉，不需資料遷移。編輯範圍只在本地項目內保存，既有封存匯入不套用外部範圍。

此功能目前僅完成本機開發，未部署。因涉及模型輸入裁切，部署驗收需在目標 GPU 上以非整頁矩形及非全黑 Mask 對所選三流程各做一次 smoke test，檢查輸入尺寸、原尺寸回貼、外圍像素與續跑；另涵蓋裁切後全黑 Mask 直通。只做輸入和機械性尺寸／回貼驗證，不作模型輸出品質拒收。遠端操作仍須先確認無 active job，6008 更新不得連帶重啟 6006；未驗證前不可宣稱 GPU 驗收完成。

## 工作流輪次介面預覽

輪次列表先切換選中，再點「前往比較合成」進入；準備頁「下一步」加大為醒目的主色按鈕。

本地使用 `npm --prefix frontend run preview:rounds`，監聽 127.0.0.1:5175，詳見 [素材準備與啟動](ROUNDS_PREVIEW.md)。此入口不需要 ComfyUI，不代理生成 API，不納入正式生產入口，也不需重啟現有 6008 服務。
...
2. 確認頁數配對無誤，再進入第二步選擇工作流；預設只選 Flux2 Klein，選一、二或三套均會生成比較 PDF（整批全黑 Mask 除外）。實際串行順序為 Flux、FireRed、Qwen。
...
只選一套或兩套時，下載包只包含選中的工作流目錄，PDF 分別命名為 `<批次名>-單工作流對比.pdf` 或 `<批次名>-雙工作流對比.pdf`，欄位為原圖 + Mask 及選中的結果。
...
選中的一、二或三套工作流全部完成後，在遠端主機直接生成比較 PDF，並與圖片、日誌一起打成單一下載包。PDF 欄位先是原圖 + Mask，再依 Flux、FireRed、Qwen 的順序放入選中的工作流；全黑 Mask 頁面省略，整批全黑不生成 PDF。
...
本次頂部快捷鍵說明與文字框剪貼也只需重建前端，沒有資料遷移。部署後確認說明位於標題列下方、工具列上方，初次開啟即顯示全部三組內容，25vh 內可捲動並可收起；窄畫面不可遮住操作按鈕。另驗收非編輯狀態的單框 ⌘／Ctrl＋C 不改系統剪貼簿、⌘／Ctrl＋V 完整貼上、⌘／Ctrl＋P 保留系統純文字換行與框樣式，且貼上中心使用左側編輯畫布指標。多選、無快照與無有效指標須只提示不新增；原位編輯中的 C／V 與 P 維持原生行為。Mac 另驗收 ⌘＋單擊，其他平台不宣稱支援此入口。
...
新增精簡頁碼導覽、起迄範圍、完成狀態、專注模式、X／Y 像素輸入、局部前後對比與 WebMCP 操作入口。部署需同時更新 6008 後端與正式前端構建；舊項目沒有完成欄位可直接讀取，資料不需遷移。部署前依既有規則檢查活動任務，更新後驗收文字保存與完成標記、修改後失效、範圍導航、原圖對比及人工操作。此功能無新增 GPU 模型或 6006 變更。

頂部狀態列改為單行並在預排版專注模式隱藏；此介面調整只需重新構建前端，沒有後端或資料遷移。

### WebMCP 分割工具驗收

`prelayout_split_item` 為前端工具擴充，無資料遷移、後端或 6006 變更。正式前端構建完成後重載已保存的 6008 工作台，確認工具重新註冊；驗收無效／過期請求不改資料、分割字元保留、撤銷還原、保存及重載一致。
