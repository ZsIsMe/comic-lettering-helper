...
未設定的頁面由前端 `defaultRepairRect` 與後端 `default_rect` 一致使用四邊 10px 內縮；即使該頁未開啟過編輯器，快照也套用相同預設。已存範圍優先；明確「本頁改為整張圖」仍可使用完整四邊。控制項併入編輯工具列，座標與操作說明改為懸停提示。
...
`projects/<id>/repair_scope.json` 是僅供本機應用使用的設定檔：`enabled` 預設 false、獨立 `revision`、`pages[page_id] = {x,y,width,height}`。前端透過內部 `PUT /api/projects/{pid}/pages/{page_id}/repair-scope` 保存手動操作；`apply_all` 覆蓋其他全部頁面的已存範圍。不同尺寸保持像素座標並裁到有效邊界。整頁操作只更新當頁，開關為項目級。保存使用項目鎖與範圍修訂檢查，並遞增項目 revision；推理／偵測／封存下載期間拒絕改範圍。獨立設定檔不進項目封存，也無 JSON 檔上傳、匯入、匯出入口；匯入項目恢復預設關閉。
...
頁碼導航預設只顯示目前頁、前後頁、範圍與完成數；完整列表可展開搜尋。範圍以一為起點的起迄頁碼保存在項目專屬 localStorage。`reviewed_revision` 記在項目頁面 metadata，與頁面 `revision` 相同且編輯器無未保存修改時才顯示完成。`PUT /api/prelayout/projects/{pid}/pages/{page_id}/review` 嚴格檢查 `expected_revision` 與布林 `reviewed`，以項目鎖更新 metadata 並回傳完整頁面；文字、匹配、譯稿或底圖更新清除標記。舊項目沒有此欄位時視為未完成，封存保留並驗證可選欄位。

WebMCP 在預排版頁註冊檢查、修改、局部對比、撤銷、保存與導航工具，依目前頁面與編輯版本核對操作；同一份草稿的自動保存不使 token 失效，內容修改、載入新版本與完成狀態變更會使其失效。工具只允許變更譯文換行，並拒絕新增頁面越界；文字邊界框相交只作提示，仍需看圖確認。來源行數只在有對應逐字偵測資料時回報，未知時不推測。局部對比以瀏覽器內的文字幾何範圍和既有底圖裁切 API 顯示編輯前／後／原圖，不另存修改前後的合成圖片。專注模式只改介面可見區，Esc 可退出；現有人工編輯與保存仍使用同一 EditorState。

共用 `app-statusbar` 收納版本、更新及 ComfyUI 控制，維持單行；預排版一般模式扣除狀態列高度，專注模式隱藏狀態列並佔滿視窗。
