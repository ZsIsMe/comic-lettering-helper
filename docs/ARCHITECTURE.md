...
`mask-edit-core.ts` 將選區套用到 overlay/other/edited：添加時兩類互斥，減去只動目前類別；框選清除會清兩類並記錄人工排除，框內互換讀取手勢開始時的兩類 Mask，純色填充轉待修補，待修補轉純色填充；空白像素及其人工編輯標記保持不變。互換不接收目前類別，與 F1／F2 無關；轉成純色時使用手勢開始時的填色。重新填色作用於本次選區，保留其他填色。原圖、後端修訂格式、Mask 方向及三套推理工作流不變。
