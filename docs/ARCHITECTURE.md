...
新工作台與舊批次入口共用 `DirectoryPicker.tsx`。使用 `showDirectoryPicker({mode: 'read'})`，`directory-files.ts` 只迭代根 handle 的 `values()`，跳過所有 directory entry，僅為符合格式的直接檔案呼叫 `getFile()`。已移除 `webkitdirectory` 與 Ant Design directory uploader；不支援的瀏覽器顯示多選圖片提示，不提供遞歸 fallback。文件夾名稱另行傳回作為預設項目名稱。

拖入文件夾走 `DataTransferItem.webkitGetAsEntry()`：只為拖入的根目錄建立 reader，多次呼叫 `readEntries()` 直到空批次，避免超過 100 個直接檔案時漏圖；不為子目錄建立 reader。一次只接受一個根目錄。取消授權、讀取失敗與零圖片都顯示狀態；讀取失敗不替換原有選擇。
