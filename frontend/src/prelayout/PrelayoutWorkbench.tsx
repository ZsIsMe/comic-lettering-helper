  ...
  return <main className="pl-shell pl-workspace">{modalContext}{noticesContext}
    ...
    <div className="pl-layout">
      ...
      <aside className="pl-inspector">
        ...
        {first ? <><p className="pl-muted">雙擊頁面文字可原位編輯，支援直排。Enter 換行；⌘／Ctrl＋Enter 完成；Esc 取消。</p><label>文字 {selected.length > 1 && `· 已選 ${selected.length} 條`}<Input.TextArea rows={5} value={first.text} onChange={e => patch({ text: e.target.value })} /></label>
