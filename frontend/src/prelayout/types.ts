...
export type Page = { id: string; name: string; width: number; height: number; revision: number; sha256: string; clean: string | null }
...
export const activeDetection = (task: Detection | null) => !!task && ['queued', 'validating', 'detecting', 'aligning', 'measuring', 'calibrating', 'publishing', 'cancelling', 'recovery_required'].includes(task.state)
