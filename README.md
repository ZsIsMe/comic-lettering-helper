...
雙擊頁面上的文字即可在原位置編輯，橫排與直排均保留現有字型、字級、描邊及旋轉。Enter 換行（直排向左另起一欄），點擊外面或 ⌘／Ctrl＋Enter 完成，Esc 取消本次輸入；完成的一次編輯可一次撤銷。保存、匯出及切換工作區會先提交原位文字。編輯期間保留中文輸入法、選字與文字複製貼上；右側文字欄仍可使用。
