...
2. **準備與編輯。** 可使用「自動檢測」或直接人工編輯；外部 Mask 僅在新建項目時匯入。畫布提供矩形、畫筆、魔法棒、套索、局部視窗，依目前 Mask 類別進行添加／減去／局部交集；魔法棒另有選區內部、選區＋選區內部、容差／擴展及點擊前預覽。右鍵框選清除兩類 Mask，Cmd／Ctrl＋右鍵互換框內的純色填充與待修補 Mask，清除另有可直接選擇的按鈕，互換僅用快捷手勢。局部副本套用後才回寫；支援縮放、平移、撤銷／重做及保存。可單獨導出底圖＋Mask。
