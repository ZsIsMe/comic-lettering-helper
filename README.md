...
預排版項目頁頂部直接顯示完整操作表，依「一般操作／文字編輯／畫面與滑鼠」分組；預設展開，內容最高約佔視窗 25%，超出可在表內捲動，也可收起以保留畫布高度。Mac 使用 ⌘／Option，Windows 使用 Ctrl／Alt；只有 Mac 支援 ⌘＋單擊進入原位編輯。

單擊選取預排版文字後，框旁提供文字顏色、描邊及橫豎排快捷切換。黑字切換成白字，其他顏色切換成黑字；描邊粗細在 0／4 間切換，描邊顏色依文字決定（黑字白描邊，其他文字黑描邊）。多選時逐框切換，可一次撤銷。原位編輯時選取部分文字，點「分割」將選中部分另建文字框放在右邊，其餘文字保留原中心；新框繼承文字樣式。撤銷分割會合回原框並保留分割前輸入的文字；沒有選取或全選時不分割。

雙擊頁面上的文字，或按住 ⌘ 單擊文字，即可在原位置編輯，橫排與直排均保留現有字型、字級、描邊及旋轉。Enter 換行（直排向左另起一欄），點擊外面或 ⌘／Ctrl＋Enter 完成，Esc 保存並結束編輯；完成的一次編輯可一次撤銷。保存、匯出及切換工作區會先提交原位文字。編輯期間保留中文輸入法、選字與文字複製貼上；右側文字欄仍可使用。
...
Option／Alt＋滾輪也可增減字級 2；普通滾輪上下捲動，⌘／Ctrl＋滾輪縮放畫面。多選時逐條增減，保留各自的字級與角度差；同一快捷鍵長按連續調整可一次撤銷，放開再按另記一步。輸入框、中文組字、彈窗及拖曳期間不觸發文字快捷鍵。PageUp／PageDown 切換頁面，Q 切換去字底圖；完整操作表常駐於網頁頂部，可展開或收起。

非編輯狀態新增三種文字框剪貼操作：⌘／Ctrl＋C 暫存單一所選框的完整內容與樣式，但不覆寫作業系統剪貼簿；⌘／Ctrl＋V 把完整框貼到左側編輯畫布的指標中心；⌘／Ctrl＋P 讀取作業系統剪貼簿純文字，保留換行並套用暫存框樣式貼上。多選不執行完整框複製；缺少暫存框或有效畫布指標時顯示提示。原位編輯中的 ⌘／Ctrl＋C／V 仍是瀏覽器原生文字剪貼，⌘／Ctrl＋P 不覆蓋編輯中的按鍵行為。
