...
範圍控制併入原有編輯工具列；說明與座標放在懸停提示。未設定過範圍的頁面預設為 `(10, 10, 圖寬-20, 圖高-20)`，已保存範圍不重設；小於 21px 的圖片縮小內縮量以保留至少 1px。

## 手動限定修復範圍（未部署）

「準備與編輯」新增預設關閉的「限定修復範圍」。開啟後手動拖曳上、下、左、右四條線，固定修復中間矩形；選中線後方向鍵逐像素微調，沒有自動偵測、吸附或內外選區。「本頁改為整張圖」只重設當頁；「將此範圍套用全部頁」覆蓋其他全部頁面，包含已單獨修改的範圍。不同尺寸沿用原圖像素座標並限制在該頁邊界內，各頁之後仍能獨立調整。

作用範圍以本地項目 JSON 保存，不提供範圍檔匯入／匯出，也不從外部項目封存套用編輯範圍。提交時固定範圍、同步裁切底圖和 Mask；生成後回貼為原尺寸圖片，範圍外保持當次底圖（含填色）。裁切後全黑 Mask 直接沿用完整底圖。此次改動尚待目標 GPU 真實回歸，未部署或發布。

## 工作流輪次本地预覽

輪次列表先切換選中，再點「前往比較合成」進入；準備頁「下一步」加大為醒目的主色按鈕。

新增獨立的本地交互預覽，可試用部分選圖、粉紅 Mask 預覽、跨工作流輪次比較及加入下一輪。無需 ComfyUI，生成僅為模擬；正式流程尚未接入。見 [預覽說明](docs/ROUNDS_PREVIEW.md)。
...
3. **批量修復。** 確認後以不可變快照提交既有引擎，按 Flux2 Klein FP8＋LanPaint、FireRed FP8、Qwen Image 2.1 INT8 串行處理，預設只選 Flux。提示詞及已調參數不變。
...
頁碼導覽位於頂部工具列下方，預設只佔一行，可切上一頁／下一頁、查看範圍完成數，並「保存並下一未完成」。展開後可設定起迄頁、搜尋檔名或頁碼、直接跳頁；範圍按項目保存在本機瀏覽器。專注模式擴大畫布，按 Esc 或「退出專注」恢復工具列。連續捲動與原圖對照仍可照常使用。
...
文字編輯欄新增 X／Y 原圖像素位置，調整多選文字時保留各框相對距離及參考框位置。逐頁完成標記只對目前保存的頁面修訂有效；修改文字、匯入譯稿、更新匹配或底圖後需重新校對。工具列的「局部前後對比」可查看受修改區域的編輯前、編輯後與原圖。支援 WebMCP 的瀏覽器可使用頁面檢查、文字修改、對比、撤銷、保存與跳頁工具；人工拖曳、輸入及保存流程不受影響。校對規則為字級對照原文，位置允許時譯文換行次數不超過對應原文。
...
本改造已納入 `0.2.10` 程式碼與更新包，尚未部署 AutoDL；不改第三部分候選合成與模型流程。驗證方式與限制見 [選區互動驗證](docs/SELECTION_RESPONSIVENESS.md)。
...
雙平台發布及更新規則見 [雙平台 Release](docs/DUAL_RELEASE.md)：同一份更新包與 SHA-256 同步 GitHub／Gitee，環境不符時拒絕套用。此次以 0.2.11 同包發布，GPU 回歸由使用者發布後開機測試。無卡檢查不能替代 GPU 驗收；有卡後先一組非空 Mask，再驗證三模型串行切換、黑 Mask 直通、時間／顯存記錄及下載。
...
預排版支援 20 色循環分組、直接切換分組及「整理分組」彈窗；可新增與重新命名，既有順序和文字 `groupId` 保持不變，不能移動或刪除。原圖與去字圖可在瀏覽器動態計算差異高亮，顏色與透明度保存於本地，並可用 `H` 切換。CTD 新增固定字級模式，保留文字框對齊、來源樣式分析與去字預覽，但跳過 OCR、逐字框分析及字級校準。

應用版本、更新與 ComfyUI 控制合併為一行狀態列；預排版專注模式會隱藏整條狀態列，畫布工作區使用完整視窗高度。
