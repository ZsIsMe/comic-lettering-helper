...
選區第二輪優化：正式編輯與保存保持順序，正式畫面優先於可丟棄的懸停預覽；左右畫面按實際像素變更區域更新，魔法棒與擴展沿用逐像素一致的加速核心。懸停預覽仍可能進行完整合成，已開始的同步計算不會被強行中斷。

矩形、畫筆、魔法棒使用不同原生游標；畫筆另保留實際尺寸圓圈。第一部分會預載目前篩選結果中的後兩頁、前一頁原圖，快取解碼資料上限 64 MiB，減少相鄰切頁時重複載入與解碼。

切頁會重用目前項目的主編輯 Worker，並轉交可移交的圖層記憶體、避免 Worker 初始化再次複製；退出編輯階段時釋放。保存與修訂檢查照常執行。完整切頁分段量測與本機真圖 A/B 結果見 [選區互動驗證](docs/SELECTION_RESPONSIVENESS.md)。
