...
本專案在 `.codex/agents/` 配置 `explorer` 和 `worker`。對需要追查程式碼路徑、跨檔分析或實作的非微小任務，主代理應主動委派具體、有明確邊界的子任務，不必等使用者每次點名模型。

- `explorer`：`gpt-6-luna`、`high`，唯讀追查入口、函式、資料流、相關測試及約束，回報檔案與行號。
- `worker`：`gpt-6-sol`、`high`，在範圍明確後實作並完成必要驗證，回報改動、測試與剩餘風險。遠端 SSH 操作也使用此模型與推理強度，遵守既有遠端操作授權與憑證規則。

需要探索才能確定實作範圍時，先取得 explorer 的結果，再指派 worker；每次委派應與主代理可獨立完成的有用工作並行。單一、明確且很小的修改可由主代理直接完成。避免多個代理同時編輯同一批檔案；主代理負責整合、審閱與最終回報。

若當前工具只有通用 `spawn_agent`，應顯式傳入上述 model 與 reasoning_effort，並在任務中交代角色、工作目錄及限制；不假設 task_name 本身會載入同名 TOML。若工具不允許指定所需模型，明確報告限制，不把其他模型稱為已配置模型。

## 漫畫排版執行方式

- 漫畫預排版、看圖校對及排版修改一律由目前主代理直接使用 Codex 內建瀏覽器執行，不啟動或委派子代理。此規則優先於上方一般子代理分工規則。
- 使用者輸入舊的 `@comic-proofreader` 名稱也不代表恢復子代理；將它理解為使用自動預排版 Skill，仍由主代理直接操作。
- 操作規範見 `/Users/zhongsheng/.codex/skills/auto-prelayout/SKILL.md`。
